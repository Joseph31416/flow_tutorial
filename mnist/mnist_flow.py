...
class Flow(nn.Module):
    def __init__(self, dim: int = 784, h: int = 2048):
        super().__init__()
...
for epoch in tqdm.tqdm(range(num_epochs)):
    ...
    for data, _ in train_loader:
        ...
        x_1 = data.view(data.size(0), -1).to(device)
...
for sample_idx in range(n_samples):
    x = torch.randn(1, 784).to(device)  # MNIST images are 28x28 = 784
    
    # plot the generated image across the time steps
    axes[sample_idx, 0].imshow(x.detach().cpu().numpy().reshape(28, 28), cmap='gray')
    axes[sample_idx, 0].set_title(f't = {time_steps[0]:.2f}')
    axes[sample_idx, 0].axis('off')
    
    for i in range(n_steps):
        x = flow.step(x, time_steps[i], time_steps[i + 1])
        axes[sample_idx, i + 1].imshow(x.detach().cpu().numpy().reshape(28, 28), cmap='gray')
